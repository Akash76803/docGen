...
describe('Phase 8.0 shape operations foundation',()=>{
  ...
  it('normalizes style values without injecting optional properties when they were absent',()=>{
    expect(normalizeDesignFill({type:'SOLID',color:'#abc',opacity:7})).toEqual({type:'SOLID',color:'#abc',opacity:1});
    expect(normalizeDesignStroke({color:'#000',widthMm:-2,style:'SOLID'})).toEqual({color:'#000',widthMm:0,style:'SOLID'});
  });
