import type { CardRenderModel, DesignBinding, DesignElement, DesignTemplate, ResolvedArtboard, ResolvedDesignElement } from '@document-tool/contracts';
...
export function resolveCardRenderModel(template:DesignTemplate, context:CardRenderResolutionContext={}, registry?:DesignElementRegistry):CardRenderResolutionResult {
  ...
  const artboards:ResolvedArtboard[]=[...template.artboards].sort((a,b)=>a.order-b.order).map(artboard=>({
    id:artboard.id,name:artboard.name,order:artboard.order,widthMm:artboard.widthMm,heightMm:artboard.heightMm,background:artboard.background,print:artboard.print,
    elements:[...artboard.elements].sort((a,b)=>a.zIndex-b.zIndex).flatMap(element=>{
      const resolved=resolveElement(element,context,warnings);
...
function resolveElement(element:DesignElement, context:CardRenderResolutionContext, warnings:string[]):ResolvedDesignElement|null {
