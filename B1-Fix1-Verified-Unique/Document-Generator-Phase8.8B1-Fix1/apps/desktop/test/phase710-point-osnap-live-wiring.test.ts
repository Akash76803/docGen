...
describe('Phase 7.10 point OSNAP live interaction wiring',()=>{
  it('limits enhanced point snapping to line/connect tools',()=>{
    expect(source).toContain("const isCadLineLikeTool=interactionMode==='PEN'||interactionMode==='FLEXIBLE_LINE'||interactionMode==='SPLIT'");
    expect(source).toContain("interactionMode==='ANGLE_LINE'||(interactionMode==='DRAW_SHAPE'&&drawShapeType==='LINE')");
  });
  ...
  it('resolves a fresh screen-space tolerance on the live canvas pointer path',()=>{
    ...
    expect(source).toContain('const tolerance=pointSnapToleranceMm(POINT_SNAP_SCREEN_TOLERANCE_PX)');
    expect(source).toContain('const snap=activeConnectTool?drawingSnap(raw,excluded,lineStart):(activeShapeDraw?shapeDrawingSnap(raw,excluded):undefined)');
  });
  ...
  it('uses the snapped point for both line preview and drag-release commit',()=>{
    ...
    expect(source).toContain('CAD LINE is intentionally click-click');
    expect(source).toContain('const upCanvasWithLineCommit=()=>{upCanvas();}');
  ...
  it('applies snapped world coordinates in the live path-node pointermove handler',()=>{
    ...
    expect(source).toContain('const delta=deltas.get(pt.id);if(!delta)return pt');
    expect(source).toContain('inHandle: pt.inHandle ? {x:pt.inHandle.x+delta.dx, y:pt.inHandle.y+delta.dy}');
  });
