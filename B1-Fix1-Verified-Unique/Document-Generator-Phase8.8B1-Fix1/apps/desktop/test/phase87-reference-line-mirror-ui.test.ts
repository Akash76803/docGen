...
describe('Phase 8.7 add-on reference-line mirror UI wiring',()=>{
 it('exposes Copy and Move CAD mirror actions',()=>{expect(toolbar).toContain('CAD Mirror Copy');expect(toolbar).toContain('CAD Mirror Move');expect(toolbar).toContain('⌁ Line Copy');expect(toolbar).toContain('⌁ Line Move');expect(toolbar).toContain('onReferenceMirrorRequested');});
