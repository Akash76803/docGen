...
describe('Phase 7.4 CAD Trimmer addendum',()=>{
 ...
 it('disables Select movement while Trimmer owns the command',()=>{expect(designer).toContain("interactionMode==='ANGLE_LINE' || interactionMode==='TRIMMER')return;capture");});
 ...
 it('prioritizes nodes, then local intersections, then nearest path',()=>{const acquire=designer.slice(designer.indexOf('const acquireTrimSnap='));const node=acquire.indexOf("kind:'NODE' as const"),intersection=acquire.indexOf("kind:'INTERSECTION'"),nearest=acquire.indexOf("kind:'NEAREST' as const");expect(node).toBeGreaterThan(0);expect(node).toBeLessThan(intersection);expect(intersection).toBeLessThan(nearest);});
 ...
 it('keeps Trimmer active after deletion and stages Escape before exit',()=>{expect(designer).not.toContain("deleteManualTrimRange();setInteractionMode('SELECT')");expect(designer).toContain('if (trimStartNodeId || trimEndNodeId || trimGeometry)');expect(designer).toContain('clearManualTrim();');expect(designer).toContain("else if(interactionMode==='EDIT_PATH'||interactionMode==='SCISSORS'||interactionMode==='TRIMMER'||interactionMode==='ERASER'||interactionMode==='FILL_BUCKET'){endHistoryTransaction();setInteractionMode('SELECT')");});
