...
describe('Phase 7.1 Trimmer UI', () => {
  ...
  it('shows Erase Segment, Polyline, and Half Circle in the existing library sections', () => {
    ...
    expect(screen.getByText('Polyline')).toBeDefined();
  ...
  it('activates Erase Segment without requiring a selected target', () => {
    const setMode = vi.fn();
  ...
  it('shows line conversions only for a compatible selected line', () => {
    ...
    expect(screen.getByText('Curve')).toBeDefined(); expect(screen.getByText('Arc')).toBeDefined();
  ...
  it('hides line-only actions and enables Flip Arc for a selected cubic', () => {
    ...
    render(<DesignerContextToolbar mode="PATH" sourceArtboard={artboard(element)} sourceElements={[element]} mutate={vi.fn()} pathEditMode={{ active: true, selectedNodeIds: [] }} interactionMode="EDIT_PATH" pathSelectedSegmentIds={['segment']} />);
    expect(screen.queryByText('Curve')).toBeNull(); expect(screen.queryByText('Arc')).toBeNull();
