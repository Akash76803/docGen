...
describe('Phase 7.4.1 CAD drawing and connection feedback',()=>{
  ...
  it('keeps strict click-click drawing and never commits LINE on pointer-up',()=>{
    ...
    expect(upCanvas?.[0]).toMatch(/explicit CAD click-click tools/i);
    expect(source).toContain('CAD LINE is intentionally click-click');
    expect(source).toContain('Pointer-up never commits a line');
