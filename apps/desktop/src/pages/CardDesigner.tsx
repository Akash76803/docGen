...
import { processImageBackground, sampleImageColor } from '../lib/imageBackgroundRemovalBrowser.js';
 ...
 const uploadImage=async(file:File)=>{if(!active)return;if(!file.type.startsWith('image/')){setStatus('Select a supported image file.');return;}try{const dataUrl=await readAsDataUrl(file);const dims=await readImageDimensions(dataUrl);const assetId=id('asset'),elementId=id('image');const ratio=dims.widthPx>0&&dims.heightPx>0?dims.widthPx/dims.heightPx:1.4;const width=Math.min(40,Math.max(15,active.widthMm*.45)),height=Math.min(active.heightMm*.6,width/ratio);mutate(t=>{let next=addAssetReference(t,{id:assetId,name:file.name,kind:'IMAGE',sourceType:'DATA_URL',source:dataUrl,mimeType:file.type,widthPx:dims.widthPx,heightPx:dims.heightPx});const placed=panelPlacement(width,height);next=addPanelAwareDesignElement(next,active.id,createImageElement(assetId,{id:elementId,name:file.name,xMm:placed.xMm,yMm:placed.yMm,widthMm:width,heightMm:height,zIndex:nextElementZIndex(next,active.id)}));return next;});selectInserted(elementId);}catch(e){setStatus(e instanceof Error?e.message:'Image upload failed');}};
    ...
    try {
      for (let i = 0; i < plan.items.length; i++) {
        ...
        try {
          ...
          const resolvedForExport=prepareArtboardForCardExport(resolved,cardRequest);
  ...
  const performExport = async () => {
    try {
      ...
      const targets = sourceTargets.map(t => prepareArtboardForCardExport(resolveArtboardBindings(t, dataContext),request));
...
function AdvancedImageProperties({element,asset,update,mutate,artboardId,availableFields,datasourceStatus}:{element:ImageDesignElement;asset?:AssetReference;update:(f:(e:DesignElement)=>DesignElement)=>void;mutate:(f:(t:DesignTemplate)=>DesignTemplate)=>void;artboardId:string;availableFields:import('@document-tool/contracts').FieldDefinition[];datasourceStatus:string}){
  ...
  const runBackgroundPreview=useCallback(async()=>{
    ...
    try{
      const result=await processImageBackground(currentSource,{mode:bgMode,tolerance:bgTolerance,backgroundColor:bgMode==='COLOR'?colorToRgb(bgColor):undefined});
      ...
      setBgMessage(`${pct}% border-connected background removed in preview.`);
    }catch(error){setBgMessage(error instanceof Error?error.message:'Background preview failed.');}
    finally{setBgProcessing(false);}
  },[canProcess,asset,currentSource,bgMode,bgTolerance,bgColor]);
  useEffect(()=>{
    setBgPreview(null);setBgPreviewEnabled(false);setBgMessage('');setBgPicking(false);
  },[element.assetId]);
  useEffect(()=>{
    if(!bgPreviewEnabled||!canProcess)return;
    const handle=window.setTimeout(()=>{void runBackgroundPreview();},220);
    return()=>window.clearTimeout(handle);
  },[bgMode,bgTolerance,bgColor,bgPreviewEnabled,canProcess,runBackgroundPreview]);
  const applyBackgroundCopy=()=>{
    if(!asset||!bgPreview)return;
    const derivedId=id('asset-bg-removed');
    const settings={mode:bgMode,tolerance:bgTolerance,backgroundColor:bgMode==='COLOR'?colorToRgb(bgColor):undefined} as const;
    const derived=createBackgroundRemovedAsset(asset,bgPreview,settings,derivedId);
  ...
  const pickPreviewColor=async(event:React.MouseEvent<HTMLImageElement>)=>{
    ...
    try{const picked=await sampleImageColor(currentSource,(event.clientX-rect.left)/Math.max(1,rect.width),(event.clientY-rect.top)/Math.max(1,rect.height));setBgMode('COLOR');setBgColor(rgbToHex(picked));setBgPicking(false);setBgMessage(`Picked ${rgbToHex(picked)}. Preview updated.`);}catch(error){setBgMessage(error instanceof Error?error.message:'Unable to sample image color.');}
  };
  ...
  <Section sectionKey="APPEARANCE" title="Background Removal">
    <div className="card-property-note"><strong>Non-destructive</strong><span>Border-connected pixels are removed; internal matching details stay protected.</span></div>
    {isBound&&<div className="card-property-note"><strong>Dynamic image detected</strong><span>Background removal is disabled while a dynamic source binding is active. Remove the binding to process the static asset; per-record removal arrives in IMG4.</span></div>}
    ...
    <div className="card-segmented-control"><button disabled={!canProcess||bgProcessing} onClick={()=>void runBackgroundPreview()}>{bgProcessing?'Processing…':'Preview Removal'}</button><button disabled={!canProcess} className={bgPicking?'active':''} onClick={()=>{setBgPicking(v=>!v);setBgMessage('Click a background pixel in the preview.');}}>Eyedropper</button></div>
    {(currentSource||bgPreview)&&<div className="card-bg-removal-preview" data-bg-removal-preview>
      <img src={bgBefore||!bgPreview?currentSource:bgPreview} alt={bgBefore?'Original image':'Background removal preview'} draggable={false} onClick={pickPreviewColor} style={{cursor:bgPicking?'crosshair':'default'}}/>
    </div>}
