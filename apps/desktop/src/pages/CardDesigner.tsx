...
import { IndexedDbDesignTemplateRepository,IndexedDbUserAssetLibraryRepository,LocalStorageArtboardPresetRepository,type CustomArtboardPreset,type UserAssetLibraryItem } from '@document-tool/persistence';
...
import { calculateSoftPathWeights, type SoftEditFalloff } from '../lib/pathSoftDeformation.js';
...
export function CardDesigner({onBack}:{onBack?:()=>void} = {}){
 const repo=useMemo(()=>new IndexedDbDesignTemplateRepository(window.localStorage),[]);
 const assetRepo=useMemo(()=>new IndexedDbUserAssetLibraryRepository(window.localStorage),[]);
 ...
 const [regroupHistory,setRegroupHistory]=useState<{artboardId:string;groups:RegroupSnapshot[]}|null>(null);
  const [interactionMode, setInteractionModeRaw] = useState<'SELECT' | 'EDIT_PATH' | 'SCISSORS' | 'PEN' | 'TRIMMER' | 'SPLIT' | 'ERASER' | 'FILL_BUCKET' | 'DRAW_SHAPE' | 'FLEXIBLE_LINE' | 'MIRROR_LINE' | 'XLINE' | 'RAY' | 'ANGLE_LINE' | 'ARC' | 'PARALLEL_LINE' | 'PERPENDICULAR_LINE' | 'REFERENCE_ALIGN'>('SELECT');
 const [referenceMirrorMode,setReferenceMirrorMode]=useState<'COPY'|'MOVE'>('COPY');
 useEffect(()=>{if(interactionMode!=='SELECT'&&panMode)setPanMode(false);if(interactionMode!=='SELECT'&&packagingPanelMode)setPackagingPanelMode(false);},[interactionMode,panMode,packagingPanelMode]);
  ...
  const [pathSelectedSegmentIds, setPathSelectedSegmentIds] = useState<string[]>([]);
   ...
   const setInteractionMode = (newMode: 'SELECT' | 'EDIT_PATH' | 'SCISSORS' | 'PEN' | 'TRIMMER' | 'SPLIT' | 'ERASER' | 'FILL_BUCKET' | 'DRAW_SHAPE' | 'FLEXIBLE_LINE' | 'MIRROR_LINE' | 'XLINE' | 'RAY' | 'ANGLE_LINE' | 'ARC' | 'PARALLEL_LINE' | 'PERPENDICULAR_LINE' | 'REFERENCE_ALIGN') => {
 ...
 const doubleDragPan=useRef<{x:number;y:number;left:number;top:number;target:Element;pointerId:number;activated:boolean}|null>(null);
 ...
 useEffect(()=>{let cancelled=false;(async()=>{try{const list=await repo.list();if(cancelled)return;setSavedTemplates(list);const activeTemplateId=await repo.getActiveId();const stored=activeTemplateId?list.find(item=>item.id===activeTemplateId)??null:list[0]??null;if(cancelled)return;if(stored){setTemplate(stored);templateRef.current=stored;resetHistory(stored);const aid=[...stored.artboards].sort((a,b)=>a.order-b.order)[0]?.id??'';setActiveId(aid);setSelection(emptySelection(aid));setStatus('Draft restored');}else{const f=fresh();setTemplate(f);templateRef.current=f;resetHistory(f);setActiveId(f.artboards[0]!.id);setSelection(emptySelection(f.artboards[0]!.id));}}catch(e){if(!cancelled){setTemplateLibraryStatus(e instanceof Error?e.message:'Unable to load saved templates.');setStatus(e instanceof Error?e.message:'Unable to restore card draft.');}}})();return()=>{cancelled=true};},[repo]);
  ...
  useEffect(()=>{try{window.localStorage.setItem('document-tool.card-font-favorites.v1',JSON.stringify(fontFavorites));}catch{/* Large legacy workspaces must never break canvas interaction. */}},[fontFavorites]);
  useEffect(()=>{try{window.localStorage.setItem('document-tool.card-font-recent.v1',JSON.stringify(recentFonts));}catch{/* Best-effort preference only. */}},[recentFonts]);
  ...
  useEffect(()=>{
    const kd=(e:KeyboardEvent)=>{
      ...
      if(e.key==='Escape'){
        const drawMode=interactionMode==='FLEXIBLE_LINE'||interactionMode==='PEN'||interactionMode==='DRAW_SHAPE'||interactionMode==='SPLIT'||interactionMode==='MIRROR_LINE'||interactionMode==='XLINE'||interactionMode==='RAY'||interactionMode==='ANGLE_LINE'||interactionMode==='ARC'||interactionMode==='PARALLEL_LINE'||interactionMode==='PERPENDICULAR_LINE'||interactionMode==='REFERENCE_ALIGN';
 ...
 const openSavedTemplate=async(templateId:string)=>{if(dirty&&!window.confirm('Discard unsaved Card Designer changes and open this saved template?'))return;try{const stored=await repo.getById(templateId);if(!stored){await refreshSavedTemplates();setTemplateLibraryStatus('Saved template was not found.');return;}setTemplate(stored);templateRef.current=stored;resetHistory(stored);const aid=[...stored.artboards].sort((a,b)=>a.order-b.order)[0]?.id??'';setActiveId(aid);setSelectedArtboardIds([]);setSelection(emptySelection(aid));setRegroupHistory(null);setZoom(100);setDirty(false);setStatus(`Opened ${stored.name}`);setTemplateLibraryStatus('');try{await repo.setActiveId(stored.id);}catch(e){const message=e instanceof Error?e.message:'Active-template preference could not be saved.';setTemplateLibraryStatus(`${stored.name} opened. ${message}`);setStatus(`Opened ${stored.name} · active preference not saved`);}}catch(e){setTemplateLibraryStatus(e instanceof Error?e.message:'Unable to open saved template.');}};
 ...
 const wheelDeltaPx=(value:number,mode:number)=>mode===1?value*16:mode===2?value*(viewport.current?.clientHeight||800):value;
 const zoomAtPointer=(event:React.WheelEvent<HTMLDivElement>)=>{event.preventDefault();const view=viewport.current;if(!view)return;const rect=view.getBoundingClientRect(),x=event.clientX-rect.left,y=event.clientY-rect.top,previous=zoom,factor=event.deltaY<0?1.14:1/1.14,next=clamp(Math.round(previous*factor),MIN_ZOOM,MAX_ZOOM);if(next===previous)return;const contentX=view.scrollLeft+x,contentY=view.scrollTop+y;setZoom(next);requestAnimationFrame(()=>{const ratio=next/previous;view.scrollLeft=contentX*ratio-x;view.scrollTop=contentY*ratio-y;});};
 const handleCanvasWheel=(event:React.WheelEvent<HTMLDivElement>)=>{const view=viewport.current;if(!view)return;if(event.ctrlKey||event.metaKey){zoomAtPointer(event);return;}event.preventDefault();const dx=wheelDeltaPx(event.deltaX,event.deltaMode),dy=wheelDeltaPx(event.deltaY,event.deltaMode);if(event.shiftKey){view.scrollLeft+=Math.abs(dx)>0.01?dx:dy;}else{view.scrollLeft+=dx;view.scrollTop+=dy;}};
 ...
 const uploadImage=async(file:File)=>{if(!active)return;if(!file.type.startsWith('image/')){setStatus('Select a supported image file.');return;}try{const dataUrl=await readAsDataUrl(file);const dims=await readImageDimensions(dataUrl);const assetId=id('asset'),elementId=id('image');const ratio=dims.widthPx>0&&dims.heightPx>0?dims.widthPx/dims.heightPx:1.4;const width=Math.min(40,Math.max(15,active.widthMm*.45)),height=Math.min(active.heightMm*.6,width/ratio);mutate(t=>{let next=addAssetReference(t,{id:assetId,name:file.name,kind:'IMAGE',sourceType:'DATA_URL',source:dataUrl,mimeType:file.type,widthPx:dims.widthPx,heightPx:dims.heightPx});const placed=panelPlacement(width,height);next=addPanelAwareDesignElement(next,active.id,createImageElement(assetId,{id:elementId,name:file.name,xMm:placed.xMm,yMm:placed.yMm,widthMm:width,heightMm:height,zIndex:nextElementZIndex(next,active.id)}));return next;});selectInserted(elementId);}catch(e){setStatus(e instanceof Error?e.message:'Image upload failed');}};
  ...
  return (
   ...
   <section className="card-canvas-column"><div className="card-canvas-toolbar"><div className="canvas-artboard-name"><MonitorUp size={15}/><strong>{active.name}</strong><span>{active.widthMm} × {active.heightMm} mm</span></div><div className="canvas-zoom-controls"><button className={leftPanelCollapsed&&inspectorCollapsed?'active':''} title="Toggle workspace panels (Tab)" onClick={()=>{const showPanels=leftPanelCollapsed&&inspectorCollapsed;setLeftPanelCollapsed(!showPanels);setInspectorCollapsed(!showPanels);setStatus(showPanels?'Workspace panels shown':'Canvas workspace expanded');}}><Maximize2 size={14}/>Canvas</button><button className={interactionMode === 'PEN' ? 'active' : ''} title="Pen Tool (Draw Paths)" style={{color: interactionMode === 'PEN' ? 'var(--accent-color)' : 'inherit', fontWeight: interactionMode === 'PEN' ? 'bold' : 'normal'}} onClick={() => setInteractionMode(interactionMode === 'PEN' ? 'SELECT' : 'PEN')}>Pen Tool</button><button className={snapEnabled?'active':''} title="Smart snapping. Hold Alt while dragging to temporarily bypass." onClick={()=>setSnapEnabled(v=>!v)}>Snap {snapEnabled?'On':'Off'}</button><label className="card-toolbar-check" title="Show measurement rulers around the artboard."><input type="checkbox" checked={showRulers} onChange={e=>setShowRulers(e.target.checked)}/>Rulers</label><label className="card-toolbar-check" title="Show editor-only grid."><input type="checkbox" checked={showGrid} onChange={e=>setShowGrid(e.target.checked)}/>Grid</label><label className="card-toolbar-check" title="Show conditionally hidden elements as ghosted."><input type="checkbox" checked={showHiddenElements} onChange={e=>setShowHiddenElements(e.target.checked)}/>Hidden</label><label className="card-toolbar-check" title="Snap elements to the configured grid."><input type="checkbox" checked={gridSnapEnabled} onChange={e=>setGridSnapEnabled(e.target.checked)}/>Snap Grid</label><label className="card-toolbar-check" title="Snap elements to custom guides."><input type="checkbox" checked={guideSnapEnabled} onChange={e=>setGuideSnapEnabled(e.target.checked)}/>Snap Guides</label><label className="card-toolbar-check" title="Show editor-only artboard and shape center guides."><input type="checkbox" checked={showSmartCenters} onChange={e=>setShowSmartCenters(e.target.checked)}/>Centers</label><label className="card-toolbar-check" title="CAD polar tracking: snap line-like drawing to configured angle increments."><input type="checkbox" checked={polarTrackingEnabled} onChange={e=>setPolarTrackingEnabled(e.target.checked)}/>Polar</label><label className="card-toolbar-check" title="CAD Ortho: constrain line-like drawing to horizontal or vertical."><input type="checkbox" checked={orthoTrackingEnabled} onChange={e=>setOrthoTrackingEnabled(e.target.checked)}/>Ortho</label><label className="card-toolbar-check" title="Track parallel/perpendicular to nearby element rotations."><input type="checkbox" checked={parallelTrackingEnabled} onChange={e=>setParallelTrackingEnabled(e.target.checked)}/>Par/Perp</label><button className={interactionMode==='PARALLEL_LINE'?'active':''} title="Parallel Line — choose an existing straight segment as reference" onClick={()=>{setInteractionMode('PARALLEL_LINE');setStatus('Parallel — hover a line or Polyline segment to choose reference');}}>∥ Parallel</button><button className={interactionMode==='PERPENDICULAR_LINE'?'active':''} title="Perpendicular Line — choose an existing straight segment as reference" onClick={()=>{setInteractionMode('PERPENDICULAR_LINE');setStatus('Perpendicular — hover a line or Polyline segment to choose reference');}}>⊥ Perp</button><label className="card-grid-size-control" title="Polar angle increment"><span>Angle</span><input type="number" min="1" max="90" step="1" value={polarIncrementDeg} onChange={e=>{const n=Number(e.target.value);if(Number.isFinite(n)&&n>=1&&n<=90)setPolarIncrementDeg(n);}}/><small>°</small></label><label className="card-grid-size-control" title="Grid spacing"><span>Grid</span><input type="number" min="0.5" step="0.5" value={normalizeDisplayValue(mmToUnit(gridSizeMm,active.displayUnit))} onChange={e=>{const next=unitToMm(Number(e.target.value),active.displayUnit);if(Number.isFinite(next)&&next>=0.5)setGridSizeMm(next);}}/><small>{active.displayUnit==='MM'?'mm':'in'}</small></label><button title="Lock or unlock all guides" className={active.guides.length&&active.guides.every(g=>g.locked)?'active':''} onClick={()=>mutate(t=>setAllGuidesLocked(t,active.id,!active.guides.every(g=>g.locked)))} disabled={!active.guides.length}>{active.guides.length&&active.guides.every(g=>g.locked)?'Unlock Guides':'Lock Guides'}</button><button title="Clear unlocked guides" onClick={()=>mutate(t=>clearGuides(t,active.id))} disabled={!active.guides.some(g=>!g.locked)}>Clear Guides</button>{packagingPanels.length>0&&<><button className={packagingPanelMode?'active':''} title="Packaging panel selection mode" onClick={()=>{setPackagingPanelMode(v=>!v);setInteractionMode('SELECT');setStatus(!packagingPanelMode?'Packaging Panels — click a panel':'Packaging Panels off');}}>Panels</button>{activePackagingPanel&&<button title={`Focus ${activePackagingPanel.name}`} onClick={()=>{setFocusedPackagingPanelId(activePackagingPanel.id);setPackagingPanelMode(false);setSelection(emptySelection(active.id));setStatus(`Focus Panel — ${activePackagingPanel.name}`);}}>Focus {activePackagingPanel.face}</button>}{focusedPackagingPanelId&&<button className="active" title="Exit focused panel (Esc)" onClick={()=>{setFocusedPackagingPanelId(null);setStatus('Panel focus exited');}}>Exit Panel</button>}{focusedPackagingPanel&&<><button disabled={!selection.elementIds.length} title="Assign selected elements to focused panel artwork" onClick={()=>{mutate(t=>assignElementsToPackagingPanel(t,active.id,selection.elementIds,focusedPackagingPanel,packagingPanels));setStatus(`Assigned ${selection.elementIds.length} element${selection.elementIds.length===1?'':'s'} to ${focusedPackagingPanel.name}`);}}>Assign</button><button disabled={!selection.elementIds.length} title="Fit selection inside panel while preserving aspect ratio" onClick={()=>mutate(t=>fitElementsToPackagingPanel(t,active.id,selection.elementIds,focusedPackagingPanel,packagingPanels,'FIT',template.sharedAssets))}>Fit</button><button disabled={!selection.elementIds.length} title="Fill focused panel" onClick={()=>mutate(t=>fitElementsToPackagingPanel(t,active.id,selection.elementIds,focusedPackagingPanel,packagingPanels,'FILL',template.sharedAssets))}>Fill</button><button disabled={!selection.elementIds.length} title="Contain selection inside focused panel" onClick={()=>mutate(t=>fitElementsToPackagingPanel(t,active.id,selection.elementIds,focusedPackagingPanel,packagingPanels,'CONTAIN',template.sharedAssets))}>Contain</button><button disabled={!selection.elementIds.length} title="Extend selected background through required bleed" onClick={()=>mutate(t=>fitElementsToPackagingPanel(t,active.id,selection.elementIds,focusedPackagingPanel,packagingPanels,'BLEED_FILL',template.sharedAssets))}>Bleed Fill</button><label title="Artwork orientation for the focused packaging panel" style={{display:'inline-flex',alignItems:'center',gap:4}}><span>Orient</span><select value={focusedPackagingPanel.artworkRotationDeg} onChange={event=>mutate(t=>setPackagingPanelArtworkOrientation(t,active.id,focusedPackagingPanel.id,Number(event.target.value),packagingPanels))}><option value={0}>0°</option><option value={90}>90°</option><option value={180}>180°</option><option value={270}>270°</option></select></label><button className={packagingIssues.length?'warning':''} title={packagingIssues.map(issue=>issue.message).join('\n')||'No packaging warnings'} onClick={()=>setStatus(packagingIssues.length?packagingIssues.slice(0,3).map(issue=>issue.message).join(' · '):'Packaging checks — no warnings')}>Warnings {packagingIssues.length}</button><button className={packagingPreflight?.errors?'warning':packagingPreflight?.warnings?'warning':''} title="Run packaging preflight" onClick={()=>setPackagingPreflightOpen(true)}>Preflight {packagingPreflight?`${packagingPreflight.errors}E/${packagingPreflight.warnings}W`:''}</button></>}</>}<button className={panMode?'active':''} title="Pan tool — drag canvas with left mouse. Middle mouse and Space+drag always pan." onClick={()=>{const next=!panMode;setPanMode(next);if(next)setInteractionMode('SELECT');setStatus(next?'Pan tool on — drag canvas':'Pan tool off');}}><Hand size={14}/>Pan</button><button title="Zoom out" onClick={()=>setZoom(z=>clamp(Math.round(z/1.25),MIN_ZOOM,MAX_ZOOM))}><Minus size={15}/></button><label title="Canvas navigation: two-finger/mouse wheel pans; Ctrl/Cmd + wheel or touchpad pinch zooms; middle mouse or Space+drag pans." style={{display:'inline-flex',alignItems:'center',gap:2}}><input aria-label="Canvas zoom percent" type="number" min={MIN_ZOOM} max={MAX_ZOOM} step={5} value={zoom} onChange={e=>{const value=Number(e.target.value);if(Number.isFinite(value))setZoom(clamp(Math.round(value),MIN_ZOOM,MAX_ZOOM));}} style={{width:58}}/><span>%</span></label><button title="Zoom in" onClick={()=>setZoom(z=>clamp(Math.round(z*1.25),MIN_ZOOM,MAX_ZOOM))}><Plus size={15}/></button><button onClick={()=>setZoom(100)}><RotateCcw size={14}/>Actual</button><button onClick={fit}><Maximize2 size={14}/>Fit</button></div></div>
    ...
    <div ref={viewport} className={`card-canvas-viewport ${(space||panMode||autoPanActive)?'pan-ready':''} ${autoPanActive?'pan-active':''}`} onWheel={handleCanvasWheel} onPointerDownCapture={e=>{const target=e.target as HTMLElement;const interactiveControl=target.closest('button,input,select,textarea,[contenteditable="true"]');const anyToolDoubleDragPan=e.button===0&&e.detail>=2&&!interactiveControl;const v=viewport.current;if(!v)return;if(anyToolDoubleDragPan){e.preventDefault();e.stopPropagation();e.currentTarget.setPointerCapture(e.pointerId);doubleDragPan.current={x:e.clientX,y:e.clientY,left:v.scrollLeft,top:v.scrollTop,target,pointerId:e.pointerId,activated:false};return;}const wantsPan=e.button===1||((space||(panMode&&interactionMode==='SELECT'))&&e.button===0);if(!wantsPan)return;e.preventDefault();e.stopPropagation();e.currentTarget.setPointerCapture(e.pointerId);pan.current={x:e.clientX,y:e.clientY,left:v.scrollLeft,top:v.scrollTop};}} onPointerMove={e=>{const v=viewport.current;if(!v)return;const candidate=doubleDragPan.current;if(candidate&&candidate.pointerId===e.pointerId){const dx=e.clientX-candidate.x,dy=e.clientY-candidate.y;if(!candidate.activated&&Math.hypot(dx,dy)>=4){candidate.activated=true;setAutoPanActive(true);setStatus(`Temporary pan — ${interactionMode} tool preserved`);}if(candidate.activated){e.preventDefault();v.scrollLeft=candidate.left-dx;v.scrollTop=candidate.top-dy;}return;}const p=pan.current;if(!p)return;e.preventDefault();v.scrollLeft=p.left-(e.clientX-p.x);v.scrollTop=p.top-(e.clientY-p.y);}} onPointerUp={e=>{const candidate=doubleDragPan.current;if(candidate&&candidate.pointerId===e.pointerId){doubleDragPan.current=null;setAutoPanActive(false);if(!candidate.activated){const synthetic=new MouseEvent('dblclick',{bubbles:true,cancelable:true,clientX:e.clientX,clientY:e.clientY,button:0});candidate.target.dispatchEvent(synthetic);}return;}pan.current=null;setAutoPanActive(false);}} onPointerCancel={e=>{if(doubleDragPan.current?.pointerId===e.pointerId)doubleDragPan.current=null;pan.current=null;setAutoPanActive(false);}}>
      ...
      {interactionMode === 'PARALLEL_LINE' && <div className="card-micro-hint" style={{position:'absolute',top:'20px',left:'50%',transform:'translateX(-50%)',background:'#2563eb',color:'white',padding:'6px 12px',borderRadius:'16px',fontSize:'12px',display:'flex',alignItems:'center',gap:'6px',pointerEvents:'none',zIndex:1000,boxShadow:'0 2px 8px rgba(0,0,0,0.2)'}}><PenLine size={14}/> PARALLEL — Hover + click reference segment → choose start/end · Esc selects.</div>}
      {interactionMode === 'PERPENDICULAR_LINE' && <div className="card-micro-hint" style={{position:'absolute',top:'20px',left:'50%',transform:'translateX(-50%)',background:'#0f766e',color:'white',padding:'6px 12px',borderRadius:'16px',fontSize:'12px',display:'flex',alignItems:'center',gap:'6px',pointerEvents:'none',zIndex:1000,boxShadow:'0 2px 8px rgba(0,0,0,0.2)'}}><PenLine size={14}/> PERPENDICULAR — Hover + click reference segment → choose start/end · Esc selects.</div>}
    ...
    <footer className="card-canvas-status"><span>{status}</span><span>{selection.elementIds.length?`${selection.elementIds.length} selected · Delete removes · Arrow nudge · Shift+Arrow 5 mm`:'Tab canvas/panels · Two-finger scroll pans · Ctrl/Cmd + scroll/pinch zooms · Space + drag pans'}</span></footer>
   </section>
    {(()=>{
...
function CardArtboardCanvas({artboard,assets,zoom,selection,setSelection,packagingPanels,packagingPanelMode,activePackagingPanelId,setActivePackagingPanelId,focusedPackagingPanelId,setFocusedPackagingPanelId,interactionMode,setInteractionMode,fillBucketType,fillBucketColor,drawShapeType,pathSelectedNodeIds,setPathSelectedNodeIds,pathSelectedSegmentIds,setPathSelectedSegmentIds,mutate,commitMutate,beginHistoryTransaction,endHistoryTransaction,snapEnabled,gridSnapEnabled,guideSnapEnabled,showRulers,showGrid,showHiddenElements,gridSizeMm,setStatus,mirrorGuideAxis,showSmartCenters,pathSymmetryMode,referenceMirrorMode,polarTrackingEnabled,orthoTrackingEnabled,parallelTrackingEnabled,polarIncrementDeg}:{artboard:Artboard;assets:DesignTemplate['sharedAssets'];zoom:number;selection:DesignSelectionState;setSelection:(s:DesignSelectionState)=>void;packagingPanels:PackagingPanel[];packagingPanelMode:boolean;activePackagingPanelId:string|null;setActivePackagingPanelId:(id:string|null)=>void;focusedPackagingPanelId:string|null;setFocusedPackagingPanelId:(id:string|null)=>void;interactionMode:'SELECT'|'EDIT_PATH'|'SCISSORS'|'PEN'|'TRIMMER'|'SPLIT'|'ERASER'|'FILL_BUCKET'|'DRAW_SHAPE'|'FLEXIBLE_LINE'|'MIRROR_LINE'|'XLINE'|'RAY'|'ANGLE_LINE'|'ARC'|'PARALLEL_LINE'|'PERPENDICULAR_LINE'|'REFERENCE_ALIGN';setInteractionMode:(m:'SELECT'|'EDIT_PATH'|'SCISSORS'|'PEN'|'TRIMMER'|'SPLIT'|'ERASER'|'FILL_BUCKET'|'DRAW_SHAPE'|'FLEXIBLE_LINE'|'MIRROR_LINE'|'XLINE'|'RAY'|'ANGLE_LINE'|'ARC'|'PARALLEL_LINE'|'PERPENDICULAR_LINE'|'REFERENCE_ALIGN')=>void;fillBucketType:'SOLID'|'NONE';fillBucketColor:string;drawShapeType:DesignShapeKind|null;pathSelectedNodeIds:string[];setPathSelectedNodeIds:React.Dispatch<React.SetStateAction<string[]>>;pathSelectedSegmentIds:string[];setPathSelectedSegmentIds:(m:string[])=>void;mutate:(f:(t:DesignTemplate)=>DesignTemplate)=>void;commitMutate:(f:(t:DesignTemplate)=>DesignTemplate)=>void;beginHistoryTransaction:()=>void;endHistoryTransaction:()=>void;snapEnabled:boolean;gridSnapEnabled:boolean;guideSnapEnabled:boolean;showRulers:boolean;showGrid:boolean;showHiddenElements:boolean;gridSizeMm:number;setStatus:(message:string)=>void;mirrorGuideAxis:'HORIZONTAL'|'VERTICAL'|null;showSmartCenters:boolean;pathSymmetryMode:'OFF'|'H'|'V';referenceMirrorMode:'COPY'|'MOVE';polarTrackingEnabled:boolean;orthoTrackingEnabled:boolean;parallelTrackingEnabled:boolean;polarIncrementDeg:number}){
 ...
 const [relationReference,setRelationReference]=useState<{elementId:string;segmentIndex:number;a:{x:number;y:number};b:{x:number;y:number};angleDeg:number;locked:boolean}|null>(null);
 useEffect(()=>{if(interactionMode!=='REFERENCE_ALIGN')setReferenceAlignPick(null);if(interactionMode!=='PARALLEL_LINE'&&interactionMode!=='PERPENDICULAR_LINE')setRelationReference(null);if(interactionMode!=='ARC')setArcDraft(null);if(interactionMode==='SELECT'){xlineReferenceRef.current=null;setCadDynamicInput({length:'',angle:'',focused:null});setCadCircleRadiusInput({radius:'',focused:false});setProjectedOrthoGuide(null);}},[interactionMode]);
 useEffect(()=>{if(interactionMode!=='DRAW_SHAPE'&&interactionMode!=='SPLIT'&&interactionMode!=='XLINE'&&interactionMode!=='RAY'&&interactionMode!=='ANGLE_LINE'&&interactionMode!=='PARALLEL_LINE'&&interactionMode!=='PERPENDICULAR_LINE'){setDrawDraft(null);if(interaction.current?.mode==='DRAW_SHAPE_DRAG')interaction.current=null;}if(interactionMode!=='PEN'&&interactionMode!=='FLEXIBLE_LINE'&&interactionMode!=='DRAW_SHAPE'&&interactionMode!=='SPLIT'&&interactionMode!=='MIRROR_LINE'&&interactionMode!=='XLINE'&&interactionMode!=='RAY'&&interactionMode!=='ANGLE_LINE'&&interactionMode!=='PARALLEL_LINE'&&interactionMode!=='PERPENDICULAR_LINE'){setBoundarySnap(null);setBoundaryHover(null);setCardinalHover(null);}if(interactionMode!=='ERASER'){eraserPointsRef.current=[];setEraserPoints([]);}if(interactionMode!=='SELECT')setSpacingGuides([]);if(interactionMode!=='MIRROR_LINE'){setReferenceMirrorDraft(null);if(interaction.current?.mode==='MIRROR_LINE')interaction.current=null;}},[interactionMode]);
 ...
 const nearestRelationReference=(raw:{xMm:number;yMm:number})=>{const tolerance=pointSnapToleranceMm(16);let best:null|{elementId:string;segmentIndex:number;a:{x:number;y:number};b:{x:number;y:number};angleDeg:number;distanceMm:number}=null;for(const el of artboard.elements){if(!el.visible||el.runtimeHidden)continue;for(const seg of elementLinearSegments(el)){const distanceMm=pointToSegmentDistance({x:raw.xMm,y:raw.yMm},seg.fromWorld,seg.toWorld);if(distanceMm<=tolerance&&(!best||distanceMm<best.distanceMm)){best={elementId:el.id,segmentIndex:seg.index,a:seg.fromWorld,b:seg.toWorld,angleDeg:normalizeAngle(Math.atan2(seg.toWorld.y-seg.fromWorld.y,seg.toWorld.x-seg.fromWorld.x)*180/Math.PI),distanceMm};}}}return best;};
 const relationTargetAngle=()=>relationReference?normalizeAngle(relationReference.angleDeg+(interactionMode==='PERPENDICULAR_LINE'?90:0)):0;
 const constrainRelationPoint=(start:{x:number;y:number},raw:{xMm:number;yMm:number})=>{const angle=relationTargetAngle(),length=Math.hypot(raw.xMm-start.x,raw.yMm-start.y),rad=angle*Math.PI/180;return{xMm:start.x+Math.cos(rad)*length,yMm:start.y+Math.sin(rad)*length};};
 const referenceProjection=(raw:{xMm:number;yMm:number})=>relationReference?projectReferencePoint({x:raw.xMm,y:raw.yMm},relationReference.a,relationReference.b):{x:raw.xMm,y:raw.yMm};
 ...
 const isCadLineLikeTool=interactionMode==='PEN'||interactionMode==='FLEXIBLE_LINE'||interactionMode==='SPLIT'||interactionMode==='MIRROR_LINE'||interactionMode==='XLINE'||interactionMode==='RAY'||interactionMode==='ANGLE_LINE'||interactionMode==='PARALLEL_LINE'||interactionMode==='PERPENDICULAR_LINE'||(interactionMode==='DRAW_SHAPE'&&drawShapeType==='LINE');
 ...
 const doubleClickIntersectionSnap=(raw:{xMm:number;yMm:number},excludeIds:string[]=[],lineStart?:{x:number;y:number})=>{
  if(!snapEnabled)return undefined;
  const tolerance=pointSnapToleranceMm(28);
  const exact=resolvePointSnap(artboard,{x:raw.xMm,y:raw.yMm},{toleranceMm:tolerance,excludeIds,snapToBoundaries:false,snapToVertices:false,snapToIntersections:true,snapToGuides:false,snapToGrid:false,snapToObjectCenters:false,snapToArtboardCenter:false,gridSizeMm});
  if(exact?.kind==='INTERSECTION')return exact;
  if(lineStart){
   const dx=raw.xMm-lineStart.x,dy=raw.yMm-lineStart.y;if(Math.hypot(dx,dy)>.001){
    const angle=normalizeAngle(Math.atan2(dy,dx)*180/Math.PI);
    const hit=cadRayIntersections(lineStart,angle,excludeIds).filter(candidate=>candidate.distanceMm>.001).sort((a,b)=>a.distanceMm-b.distanceMm)[0];
    if(hit)return {point:{x:hit.x,y:hit.y},kind:'INTERSECTION' as const,label:'Nearest Intersection',distanceMm:Math.hypot(hit.x-raw.xMm,hit.y-raw.yMm),elementId:hit.elementId,detailId:'DOUBLE_CLICK_NEAREST_INTERSECTION'};
   }
  }
  return undefined;
 };
 const moveOpenLinearPathEndpointToWorld=(element:PathDesignElement,pointId:string,target:{x:number;y:number})=>{
  const world=element.geometry.points.map(point=>({point,x:element.position.xMm+point.x,y:element.position.yMm+point.y}));
  const replaced=world.map(item=>item.point.id===pointId?{...item,x:target.x,y:target.y}:item);
  const minX=Math.min(...replaced.map(item=>item.x)),minY=Math.min(...replaced.map(item=>item.y));
  const maxX=Math.max(...replaced.map(item=>item.x)),maxY=Math.max(...replaced.map(item=>item.y));
  const points=replaced.map(item=>{
   const dx=item.x-(element.position.xMm+item.point.x),dy=item.y-(element.position.yMm+item.point.y);
   return {...item.point,x:item.x-minX,y:item.y-minY,inHandle:item.point.inHandle?{x:item.point.inHandle.x+dx+(element.position.xMm-minX),y:item.point.inHandle.y+dy+(element.position.yMm-minY)}:undefined,outHandle:item.point.outHandle?{x:item.point.outHandle.x+dx+(element.position.xMm-minX),y:item.point.outHandle.y+dy+(element.position.yMm-minY)}:undefined};
  });
  return {...element,position:{xMm:minX,yMm:minY},size:{widthMm:Math.max(.1,maxX-minX),heightMm:Math.max(.1,maxY-minY)},geometry:{...element.geometry,points}};
 };
 ...
 const commitDrawDraft=(draft:DrawDraft,end:{xMm:number;yMm:number},endSnap?:PointSnapResult)=>{
   ...
   endHistoryTransaction();
   if(draft.shapeType==='LINE'&&!splitOnly&&interactionMode==='DRAW_SHAPE'){
   ...
   }else{interaction.current=null;setDrawDraft(null);if(interactionMode==='ANGLE_LINE')setStatus('ANGLE LINE — Specify next start point');else if(interactionMode==='PARALLEL_LINE'||interactionMode==='PERPENDICULAR_LINE')setStatus(`${interactionMode==='PARALLEL_LINE'?'Parallel':'Perpendicular'} — reference locked · choose next start point`);else{setInteractionMode('SELECT');setStatus(`${draft.shapeType} created — selected for resize, rotation and point inspection`);}}
 };
 const commitDynamicCadLine=()=>{
   if(!((interactionMode==='DRAW_SHAPE'&&drawShapeType==='LINE')||interactionMode==='ANGLE_LINE'||interactionMode==='PARALLEL_LINE'||interactionMode==='PERPENDICULAR_LINE')||!drawDraft)return;
   ...
   const angleDeg=(interactionMode==='PARALLEL_LINE'||interactionMode==='PERPENDICULAR_LINE')&&relationReference?.locked?relationTargetAngle():(Number.isFinite(parsedAngle)?normalizeAngle(parsedAngle):(tracked.label?tracked.angleDeg:normalizeAngle(Math.atan2(liveDy,liveDx)*180/Math.PI)));
   if(!Number.isFinite(lengthMm)||lengthMm<0.01){setStatus(`${interactionMode==='ANGLE_LINE'?'ANGLE LINE':interactionMode==='PARALLEL_LINE'?'PARALLEL':interactionMode==='PERPENDICULAR_LINE'?'PERPENDICULAR':'LINE'} — enter a length greater than 0`);return;}
   const end=resolveCadDynamicEndpoint({xMm:drawDraft.startX,yMm:drawDraft.startY},lengthMm,angleDeg);
   commitDrawDraft(drawDraft,end,undefined);setCadDynamicInput({length:'',angle:'',focused:null});setStatus(`${interactionMode==='ANGLE_LINE'?'ANGLE LINE':interactionMode==='PARALLEL_LINE'?'PARALLEL':interactionMode==='PERPENDICULAR_LINE'?'PERPENDICULAR':'LINE'} — ${lengthMm.toFixed(2)} mm @ ${angleDeg.toFixed(2)}° committed`);
 };
 ...
 const toolDownCapture=(e:React.PointerEvent<HTMLDivElement>)=>{
  ...
  const interactiveControl=target.closest('button,input,select,textarea,[contenteditable="true"]');
  // VECTOR-UX2 Fix8: Drawing tools must win pointer routing in capture phase.
  // This prevents transparent overlays / selected elements / HUD shells from swallowing
  // LINE/POLYLINE (and sibling CAD tool) clicks before downCanvas receives them.
  const capturePriorityDraw=
    interactionMode==='ARC'||
    interactionMode==='PEN'||
    interactionMode==='FLEXIBLE_LINE'||
    interactionMode==='SPLIT'||
    interactionMode==='XLINE'||
    interactionMode==='RAY'||
    interactionMode==='ANGLE_LINE'||
    interactionMode==='PARALLEL_LINE'||
    interactionMode==='PERPENDICULAR_LINE'||
    interactionMode==='MIRROR_LINE'||
    (interactionMode==='DRAW_SHAPE'&&!!drawShapeType);
  if(capturePriorityDraw&&!interactiveControl){
  ...
  if(interactionMode==='FILL_BUCKET'){
   ...
   const region=findJoinedLineRegionAtPoint(artboard.elements,{x:p.xMm,y:p.yMm},{widthMm:artboard.widthMm,heightMm:artboard.heightMm,enabled:pageBorderSettings(artboard).enabled});
   if(region){
    const newId=id('joined-line-section'),componentId=id('face-component');
    const fill=fillBucketType==='NONE'?{type:'NONE' as const}:{type:'SOLID' as const,color:fillBucketColor,opacity:1};
    const sourceElements=artboard.elements.filter(element=>region.sourceElementIds.includes(element.id));
    const filledSourceZ=sourceElements.filter(element=>(element.type==='PATH'||element.type==='SHAPE')&&element.fill?.type!=='NONE').map(element=>element.zIndex);
    const sectionZ=filledSourceZ.length?Math.min(...filledSourceZ)-0.001:Math.max(-1,...sourceElements.map(element=>element.zIndex),...artboard.elements.map(element=>element.zIndex))+0.001;
    const section:PathDesignElement={id:newId,type:'PATH',name:'Joined Line Section',position:region.position,size:region.size,rotationDeg:0,opacity:1,zIndex:sectionZ,visible:true,locked:false,geometry:region.geometry,fill,stroke:{style:'SOLID',color:'#000000',widthMm:.5,opacity:1},metadata:{faceComponentId:componentId,faceGeneration:'AUTO_SECTION',faceTopologyVersion:2,joinedLineSourceIds:region.sourceElementIds,preserveSourceFills:true}};
 ...
 const downCanvas=(e:React.PointerEvent<HTMLDivElement>)=>{
  ...
  if(interactionMode==='PARALLEL_LINE'||interactionMode==='PERPENDICULAR_LINE'){
    if(e.button!==0)return;
    const raw=point(e);
    if(!relationReference?.locked){const ref=nearestRelationReference(raw);if(!ref){setStatus(`${interactionMode==='PARALLEL_LINE'?'Parallel':'Perpendicular'} — hover a straight line or Polyline segment`);return;}setRelationReference({...ref,locked:true});setStatus(`${interactionMode==='PARALLEL_LINE'?'Parallel':'Perpendicular'} — reference locked @ ${ref.angleDeg.toFixed(2)}° · choose start point`);return;}
    const existing=interaction.current?.mode==='DRAW_SHAPE_DRAG'?interaction.current:null;
    if(!existing){let startRaw=raw;if(interactionMode==='PERPENDICULAR_LINE'){const projected=referenceProjection(raw);const nearRef=pointToSegmentDistance({x:raw.xMm,y:raw.yMm},relationReference.a,relationReference.b)<=pointSnapToleranceMm(16);if(nearRef)startRaw={xMm:projected.x,yMm:projected.y};}const snap=drawingSnap(startRaw,[]);const p=snap?{xMm:snap.point.x,yMm:snap.point.y}:startRaw;beginHistoryTransaction();const draft:DrawDraft={startX:p.xMm,startY:p.yMm,currentX:p.xMm,currentY:p.yMm,shapeType:'LINE',intent:'DRAW',isShift:false,pointerIsDown:false,movedDuringPress:false,startSnap:snap};interaction.current={mode:'DRAW_SHAPE_DRAG',...draft};setDrawDraft(draft);setBoundarySnap(snap??null);setStatus(`${interactionMode==='PARALLEL_LINE'?'Parallel':'Perpendicular'} — ${relationTargetAngle().toFixed(2)}° locked · choose endpoint`);return;}
    const constrained=constrainRelationPoint({x:existing.startX,y:existing.startY},raw);const snap=drawingSnap(constrained,[],{x:existing.startX,y:existing.startY});const p=snap?{xMm:snap.point.x,yMm:snap.point.y}:constrained;commitDrawDraft(existing,p,snap);setCadDynamicInput({length:'',angle:'',focused:null});return;
  }
    ...
    if (selection.elementIds.length === 1) {
      ...
      if (selectedEl && selectedEl.type === 'PATH' && (interactionMode!=='FLEXIBLE_LINE'||selectedEl.metadata?.cadGeometryKind==='POLYLINE')) {
        ...
        if (lastNode) {
          ...
          const closePolyline = interactionMode==='FLEXIBLE_LINE' && cadPolylineCanCloseAtPoint(selectedEl as PathDesignElement,{xMm:p.xMm,yMm:p.yMm},pointSnapToleranceMm(14));
          commitMutate(t => {
            const extended={
              ...t, artboards: t.artboards.map(a => a.id === artboard.id ? {
                ...a, elements: a.elements.map(el => {
                  if (el.id !== selectedId) return el;
                  const pEl = el as PathDesignElement;
                  if(interactionMode==='FLEXIBLE_LINE'){
                    if(closePolyline)return closeCadPolyline(pEl);
                    const appended=appendCadPolylinePoint(pEl,{xMm:p.xMm,yMm:p.yMm},newPtId);return {...appended,metadata:{...appended.metadata,...(snap?.elementId?{cadEndTargetId:snap.elementId}:{})}};
                  }
      ...
      if(interactionMode==='FLEXIBLE_LINE' && selection.elementIds.length===1){
        const selectedPolyline=artboard.elements.find(el=>el.id===selection.elementIds[0]);
        if(selectedPolyline?.type==='PATH'&&cadPolylineCanCloseAtPoint(selectedPolyline,{xMm:p.xMm,yMm:p.yMm},pointSnapToleranceMm(14))){
          endHistoryTransaction();setInteractionMode('SELECT');setPenHover(null);setBoundarySnap(null);setStatus('Polyline — closed path created');interaction.current=null;return;
        }
      }
 ...
 return <div ref={canvas} className={`card-artboard-canvas ${showRulers?'with-rulers':''}`} data-artboard-id={artboard.id} style={canvasStyle} onPointerDownCapture={toolDownCapture} onPointerDown={downCanvas} onPointerMoveCapture={moveArcCapture} onPointerMove={moveCanvas} onPointerUp={upCanvasWithLineCommit} onDoubleClick={ev=>{const target=ev.target as HTMLElement;const interactiveControl=target.closest('button,input,select,textarea,[contenteditable="true"]');if(interactiveControl)return;const raw=point(ev as unknown as React.PointerEvent<HTMLDivElement>);if(interactionMode==='FLEXIBLE_LINE'){ev.preventDefault();const activeId=selection.primaryElementId??selection.elementIds[0];const active=activeId?artboard.elements.find(element=>element.id===activeId):undefined;const excluded=activeId?[activeId]:[];const polylinePoints=active?.type==='PATH'?active.geometry.points:[];const previousPoint=active?.type==='PATH'&&polylinePoints.length>1?polylinePoints[polylinePoints.length-2]:undefined;const lineStart=active?.type==='PATH'&&previousPoint?{x:active.position.xMm+previousPoint.x,y:active.position.yMm+previousPoint.y}:undefined;const snap=doubleClickIntersectionSnap(raw,excluded,lineStart);if(active?.type==='PATH'&&active.metadata?.cadGeometryKind==='POLYLINE'&&active.geometry.points.length){const endpoints=getPathEndpoints(active.geometry);const endpointId=endpoints[endpoints.length-1]??active.geometry.points[active.geometry.points.length-1]?.id;if(endpointId&&snap){commitMutate(template=>({...template,artboards:template.artboards.map(current=>current.id!==artboard.id?current:{...current,elements:materializeStraightPathIntersections(current.elements.map(element=>element.id===active.id?{...moveOpenLinearPathEndpointToWorld(element as PathDesignElement,endpointId,snap.point),metadata:{...element.metadata,cadEndTargetId:snap.elementId}}:element),[active.id])})}));setStatus(`Polyline — finished at ${snap.label??'nearest intersection'}`);}else setStatus('Polyline — finished');}endHistoryTransaction();interaction.current=null;setSelection(emptySelection(artboard.id));setPenHover(null);setBoundarySnap(null);intersectionCaptureLockRef.current=null;return;}if(interactionMode==='DRAW_SHAPE'&&drawShapeType==='LINE'){ev.preventDefault();const activeId=selection.primaryElementId??selection.elementIds[0];const active=activeId?artboard.elements.find(element=>element.id===activeId):undefined;const lineFirstPoint=active?.type==='PATH'?active.geometry.points[0]:undefined;const lineStart=active?.type==='PATH'&&lineFirstPoint?{x:active.position.xMm+lineFirstPoint.x,y:active.position.yMm+lineFirstPoint.y}:undefined;const snap=doubleClickIntersectionSnap(raw,activeId?[activeId]:[],lineStart);if(active?.type==='PATH'&&active.geometry.points.length>=2&&snap){const endpoints=getPathEndpoints(active.geometry);const endpointId=endpoints[endpoints.length-1]??active.geometry.points[active.geometry.points.length-1]?.id;if(endpointId){commitMutate(template=>({...template,artboards:template.artboards.map(current=>current.id!==artboard.id?current:{...current,elements:materializeStraightPathIntersections(current.elements.map(element=>element.id===active.id?{...moveOpenLinearPathEndpointToWorld(element as PathDesignElement,endpointId,snap.point),metadata:{...element.metadata,cadEndTargetId:snap.elementId}}:element),[active.id])})}));setStatus(`LINE — finished at ${snap.label??'nearest intersection'} · Specify first point`);}}else setStatus('LINE — chain finished · Specify first point');interaction.current=null;setDrawDraft(null);setBoundarySnap(null);setBoundaryHover(null);setCardinalHover(null);intersectionCaptureLockRef.current=null;setCadDynamicInput({length:'',angle:'',focused:null});}}} onPointerCancel={cancelEraserStroke}>
  <ArtboardBackgroundVisual artboard={artboard} assets={assets}/>
  <PageBorderVisual artboard={artboard} zoom={zoom}/>
  ...
  {relationReference&&<div data-cad-relation-reference data-mode={interactionMode} style={{position:'absolute',inset:0,pointerEvents:'none',zIndex:100006}}><svg width="100%" height="100%" viewBox={`0 0 ${artboard.widthMm} ${artboard.heightMm}`} preserveAspectRatio="none"><line x1={relationReference.a.x} y1={relationReference.a.y} x2={relationReference.b.x} y2={relationReference.b.y} stroke={relationReference.locked?'#f59e0b':'#fb923c'} strokeWidth={(relationReference.locked?2:1.4)/MM_TO_CSS_PX/(zoom/100)} vectorEffect="non-scaling-stroke"/>{relationReference.locked&&(()=>{const start=drawDraft?{x:drawDraft.startX,y:drawDraft.startY}:referenceProjection({xMm:(relationReference.a.x+relationReference.b.x)/2,yMm:(relationReference.a.y+relationReference.b.y)/2});const angle=relationTargetAngle(),rad=angle*Math.PI/180,length=Math.hypot(artboard.widthMm,artboard.heightMm)*2;return <g><line x1={start.x-Math.cos(rad)*length} y1={start.y-Math.sin(rad)*length} x2={start.x+Math.cos(rad)*length} y2={start.y+Math.sin(rad)*length} stroke="#2563eb" strokeWidth={1.2/MM_TO_CSS_PX/(zoom/100)} strokeDasharray={`${2/MM_TO_CSS_PX/(zoom/100)} ${2/MM_TO_CSS_PX/(zoom/100)}`} vectorEffect="non-scaling-stroke"/>{interactionMode==='PERPENDICULAR_LINE'&&<g data-right-angle-marker><rect x={start.x-2.5/MM_TO_CSS_PX/(zoom/100)} y={start.y-2.5/MM_TO_CSS_PX/(zoom/100)} width={5/MM_TO_CSS_PX/(zoom/100)} height={5/MM_TO_CSS_PX/(zoom/100)} fill="none" stroke="#10b981" strokeWidth={1/MM_TO_CSS_PX/(zoom/100)} vectorEffect="non-scaling-stroke"/></g>}<text x={start.x} y={start.y} dx={6/MM_TO_CSS_PX/(zoom/100)} dy={-6/MM_TO_CSS_PX/(zoom/100)} fill="#1d4ed8" fontSize={9/MM_TO_CSS_PX/(zoom/100)}>{interactionMode==='PARALLEL_LINE'?`∥ Parallel · Offset ${pointToSegmentDistance(start,relationReference.a,relationReference.b).toFixed(2)} mm`:'⊥ 90°'}</text></g>;})()}</svg></div>}
...
function PathNodeEditor({element, zoom, interactionMode, pathSelectedNodeIds, setPathSelectedNodeIds, pathSelectedSegmentIds, setPathSelectedSegmentIds, mutate, artboardId, beginHistoryTransaction, endHistoryTransaction, onTrimGeometry, allElements, artboard, snapEnabled, gridSnapEnabled, guideSnapEnabled, gridSizeMm,showSmartCenters,symmetryMode}: {element: PathDesignElement; zoom: number; interactionMode: string; pathSelectedNodeIds: string[]; setPathSelectedNodeIds: React.Dispatch<React.SetStateAction<string[]>>; pathSelectedSegmentIds: string[]; setPathSelectedSegmentIds: (m: string[]) => void; mutate: (f: (t: DesignTemplate) => DesignTemplate) => void; artboardId: string; beginHistoryTransaction: () => void; endHistoryTransaction: () => void; onTrimGeometry: (geometry: PathDesignElement['geometry']) => void; allElements?: DesignElement[]; artboard:Artboard; snapEnabled:boolean; gridSnapEnabled:boolean; guideSnapEnabled:boolean; gridSizeMm:number; showSmartCenters:boolean; symmetryMode:'OFF'|'H'|'V'}) {
  const [softEditEnabled,setSoftEditEnabled]=useState(true);
  const [softEditRadiusMm,setSoftEditRadiusMm]=useState(45);
  const [softEditStrengthPct,setSoftEditStrengthPct]=useState(100);
  const [softEditFalloff,setSoftEditFalloff]=useState<SoftEditFalloff>('SMOOTH');
  const [softEditPreserveEnds,setSoftEditPreserveEnds]=useState(true);
  ...
  const [pathHoverSegmentId,setPathHoverSegmentId]=useState<string|null>(null);
  ...
  return <div className="card-path-node-editor" style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',cursor:interactionMode==='TRIMMER'?TRIMMER_CURSOR:undefined}}>
    {interactionMode==='EDIT_PATH'&&<div data-soft-path-edit-controls onPointerDown={event=>event.stopPropagation()} onDoubleClick={event=>event.stopPropagation()} style={{position:'absolute',right:0,top:-44,zIndex:35,display:'flex',alignItems:'center',gap:6,whiteSpace:'nowrap',background:'var(--bg-primary)',border:'1px solid var(--border-color)',borderRadius:6,padding:'5px 7px',fontSize:10,boxShadow:'0 2px 8px rgba(0,0,0,.12)'}}>
      <label title="Move nearby nodes with a smooth falloff along the path"><input type="checkbox" checked={softEditEnabled} onChange={event=>setSoftEditEnabled(event.target.checked)}/> Soft Edit</label>
      {softEditEnabled&&<>
        <label>Radius <input aria-label="Soft edit influence radius mm" type="number" min={1} max={1000} step={1} value={softEditRadiusMm} onChange={event=>setSoftEditRadiusMm(clamp(Number(event.target.value)||1,1,1000))} style={{width:52}}/> mm</label>
        <label>Falloff <select aria-label="Soft edit falloff" value={softEditFalloff} onChange={event=>setSoftEditFalloff(event.target.value as SoftEditFalloff)}><option value="SMOOTH">Smooth</option><option value="GAUSSIAN">Gaussian</option><option value="LINEAR">Linear</option></select></label>
        <label>Strength <input aria-label="Soft edit strength percent" type="number" min={1} max={100} step={5} value={softEditStrengthPct} onChange={event=>setSoftEditStrengthPct(clamp(Number(event.target.value)||1,1,100))} style={{width:46}}/>%</label>
        <label title="Keep open-path end nodes fixed unless directly selected"><input type="checkbox" checked={softEditPreserveEnds} onChange={event=>setSoftEditPreserveEnds(event.target.checked)}/> Preserve Ends</label>
      </>}
    </div>}
    ...
    <svg style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',pointerEvents:'none',overflow:'visible'}} viewBox={`0 0 ${element.size.widthMm} ${element.size.heightMm}`} preserveAspectRatio="none">
      ...
      {activeGeometry.segments.map(seg => {
        ...
        const isHoveredForEdit = interactionMode==='EDIT_PATH' && pathHoverSegmentId===seg.id;
        ...
        return <g key={seg.id || `seg-${seg.fromPointId}-${seg.toPointId}`}>
          {isHoveredForEdit && !isSelected && <path data-path-segment-hover d={d} fill="none" stroke="#f59e0b" strokeWidth={2.4/MM_TO_CSS_PX/(zoom/100)} pointerEvents="none" opacity={.9} />}
          {isSelected && <path data-path-segment-selected d={d} fill="none" stroke="var(--accent-color)" strokeWidth={2.8/MM_TO_CSS_PX/(zoom/100)} pointerEvents="none" />}
          ...
          <path data-path-segment-hit-target d={d} fill="none" stroke="transparent" strokeWidth={(interactionMode==='TRIMMER'?14:interactionMode==='EDIT_PATH'?14:10)/MM_TO_CSS_PX/(zoom/100)} vectorEffect="non-scaling-stroke" pointerEvents="stroke" style={{cursor: interactionMode === 'TRIMMER' ? TRIMMER_CURSOR : interactionMode === 'SCISSORS' ? 'crosshair' : 'pointer'}} 
            onPointerEnter={(ev) => {
              if(interactionMode==='EDIT_PATH')setPathHoverSegmentId(seg.id as string);
            ...
            onPointerLeave={() => { if(interactionMode==='EDIT_PATH')setPathHoverSegmentId(current=>current===seg.id?null:current); if (interactionMode === 'TRIMMER') { setTrimmerHover(null); setHoveredInterval(null); setTrimSnap(null); } }}
            onPointerDown={(ev) => {
              ...
              if (interactionMode === 'SCISSORS') {
                ...
                if(interactionMode==='EDIT_PATH'&&(ev.ctrlKey||ev.metaKey))setPathSelectedSegmentIds(pathSelectedSegmentIds.includes(segmentId)?pathSelectedSegmentIds.filter(id=>id!==segmentId):[...pathSelectedSegmentIds,segmentId]);
                else setPathSelectedSegmentIds([segmentId]);
                setPathSelectedNodeIds([]);
              }
            }}
            onDoubleClick={(ev)=>{
    ...
    {activeGeometry.points.map(p => {
      ...
      return <div key={`node-${p.id}`}>
        ...
        <i data-path-node-id={p.id} data-intersection-node={Array.isArray(element.metadata?.intersectionNodeIds)&&element.metadata.intersectionNodeIds.includes(p.id)?'true':undefined} title={Array.isArray(element.metadata?.intersectionNodeIds)&&element.metadata.intersectionNodeIds.includes(p.id)?'Intersection node':'Path node'} className={`card-path-node ${isSelected?'selected':''}`} style={{position:'absolute',left:p.x*MM_TO_CSS_PX,top:p.y*MM_TO_CSS_PX,width:interactionMode==='TRIMMER'?trimNodeHitSize:(isSelected?14:11),height:interactionMode==='TRIMMER'?trimNodeHitSize:(isSelected?14:11),marginLeft:interactionMode==='TRIMMER'?-trimNodeHitSize/2:(isSelected?-7:-5.5),marginTop:interactionMode==='TRIMMER'?-trimNodeHitSize/2:(isSelected?-7:-5.5),background:interactionMode==='TRIMMER'?`radial-gradient(circle, ${p.id===trimStartNodeId?'#22c55e':p.id===trimEndNodeId?'#3b82f6':'white'} 0 3px, var(--accent-color) 3px 4px, transparent 4px)`:undefined,backgroundColor:interactionMode==='TRIMMER'?undefined:(Array.isArray(element.metadata?.intersectionNodeIds)&&element.metadata.intersectionNodeIds.includes(p.id))?'#22c55e':isSelected?'#ef4444':'white',border:interactionMode==='TRIMMER'?'none':(Array.isArray(element.metadata?.intersectionNodeIds)&&element.metadata.intersectionNodeIds.includes(p.id))?'2px solid #15803d':isSelected?'2px solid #b91c1c':'1.5px solid #ef4444',boxShadow:interactionMode==='TRIMMER'?undefined:(Array.isArray(element.metadata?.intersectionNodeIds)&&element.metadata.intersectionNodeIds.includes(p.id))?'0 0 0 3px rgba(34,197,94,.2)':isSelected?'0 0 0 2px rgba(255,255,255,.9),0 0 0 3px rgba(239,68,68,.35)':undefined,borderRadius:'50%',cursor:interactionMode==='TRIMMER'?TRIMMER_CURSOR:'pointer',zIndex:interactionMode==='TRIMMER'?10:12}} onDoubleClick={ev=>{if(interactionMode==='EDIT_PATH'){ev.preventDefault();ev.stopPropagation();if(extendCadLineEndpointToBoundary(p.id))return;}}} onPointerDown={ev=>{
          ev.stopPropagation();
          ...
          const softWeights=softEditEnabled?calculateSoftPathWeights(element.geometry,dragIds,{radiusMm:softEditRadiusMm,strength:softEditStrengthPct/100,falloff:softEditFalloff,preserveEnds:softEditPreserveEnds}):null;
          ...
          const move = (eMove: PointerEvent) => {
            ...
            mutate(t => {
              ...
              if(softWeights){
                for(const point of el.geometry.points){
                  const weight=softWeights.get(point.id)??0;
                  if(weight>0.0001)deltas.set(point.id,{dx:dx*weight,dy:dy*weight});
                }
              }else{
...
type PageBorderSettings={enabled:boolean;color:string;widthPx:number;style:'SOLID'|'DASHED'|'DOTTED';position:'INSIDE'|'CENTER'|'OUTSIDE';exportEnabled:boolean};
const DEFAULT_PAGE_BORDER:PageBorderSettings={enabled:true,color:'#64748b',widthPx:1,style:'SOLID',position:'INSIDE',exportEnabled:false};
function pageBorderSettings(artboard:Artboard):PageBorderSettings{const raw=(artboard.metadata?.pageBorder??{}) as Partial<PageBorderSettings>;return{enabled:raw.enabled??DEFAULT_PAGE_BORDER.enabled,color:typeof raw.color==='string'?raw.color:DEFAULT_PAGE_BORDER.color,widthPx:Number.isFinite(raw.widthPx)?Math.max(.25,Math.min(12,Number(raw.widthPx))):DEFAULT_PAGE_BORDER.widthPx,style:raw.style==='DASHED'||raw.style==='DOTTED'?raw.style:'SOLID',position:raw.position==='CENTER'||raw.position==='OUTSIDE'?raw.position:'INSIDE',exportEnabled:raw.exportEnabled??false};}
function PageBorderVisual({artboard,zoom}:{artboard:Artboard;zoom:number}){const b=pageBorderSettings(artboard);if(!b.enabled)return null;const scale=Math.max(.01,zoom/100),w=b.widthPx/scale,offset=b.position==='INSIDE'?w/2:b.position==='OUTSIDE'?-w/2:0;return <div data-page-border style={{position:'absolute',left:offset,top:offset,right:offset,bottom:offset,border:`${w}px ${b.style==='DASHED'?'dashed':b.style==='DOTTED'?'dotted':'solid'} ${b.color}`,boxSizing:'border-box',pointerEvents:'none',zIndex:100050}}/>;}
...
function ElementProperties({element,asset,assets,template,artboard,mutate,availableFields,datasourceStatus}:{element:DesignElement;asset?:AssetReference;assets:DesignTemplate['sharedAssets'];template:DesignTemplate;artboard:Artboard;mutate:(f:(t:DesignTemplate)=>DesignTemplate)=>void;availableFields:import('@document-tool/contracts').FieldDefinition[];datasourceStatus:string}){
...
function TextLayerEffectsControls({effects,onChange,onMigrate,globalLight,onGlobalLightChange}:{effects:TextLayerEffect[]|undefined;onChange:(effects:TextLayerEffect[])=>void;onMigrate:()=>void;globalLight:{angleDeg:number;altitudeDeg:number};onGlobalLightChange:(angleDeg:number,altitudeDeg:number)=>void}){
  const normalized=normalizeTextLayerEffects(effects);
  const [addType,setAddType]=useState<TextLayerEffectType>('STROKE');
  const labels:Record<TextLayerEffectType,string>={STROKE:'Stroke',COLOR_OVERLAY:'Color Overlay',GRADIENT_OVERLAY:'Gradient Overlay',PATTERN_OVERLAY:'Pattern Overlay',INNER_SHADOW:'Inner Shadow',INNER_GLOW:'Inner Glow',OUTER_GLOW:'Outer Glow',DROP_SHADOW:'Drop Shadow',BEVEL_EMBOSS:'Bevel & Emboss',SATIN:'Satin'};
  ...
  return <div className="card-text-layer-effects-engine" data-text6a-layer-effects>
    ...
    {normalized.map((effect,index)=><details key={effect.id} className="card-text-layer-effect-row" open={index===0}>
      ...
      <div className="card-property-details">
        ...
        {(effect.type==='INNER_GLOW'||effect.type==='OUTER_GLOW')&&<><label>Color<input type="color" value={effect.settings.color??'#ffffff'} onChange={e=>patchSettings(effect.id,{color:e.target.value})}/></label><div className="card-property-grid"><label>Size / Blur<input type="number" min="0" step=".05" value={effect.settings.blurMm??.8} onChange={e=>patchSettings(effect.id,{blurMm:Math.max(0,Number(e.target.value)||0)})}/></label><label>Spread %<input type="number" min="0" max="100" value={Math.round((effect.settings.spread??0)*100)} onChange={e=>patchSettings(effect.id,{spread:clamp(Number(e.target.value)||0,0,100)/100})}/></label></div></>}{(effect.type==='INNER_GLOW'||effect.type==='OUTER_GLOW'||effect.type==='DROP_SHADOW'||effect.type==='INNER_SHADOW')&&<div className="card-property-grid"><label>Noise %<input type="number" min="0" max="100" value={Math.round((effect.settings.noise??0)*100)} onChange={e=>patchSettings(effect.id,{noise:clamp(Number(e.target.value)||0,0,100)/100})}/></label><label>Choke %<input type="number" min="0" max="100" value={Math.round((effect.settings.choke??0)*100)} onChange={e=>patchSettings(effect.id,{choke:clamp(Number(e.target.value)||0,0,100)/100})}/></label></div>}{effect.type==='SATIN'&&<><div className="card-property-note"><strong>Satin</strong><span>Creates internal contour shading for glossy and metallic surfaces.</span></div><div className="card-property-grid"><label>Color<input type="color" value={effect.settings.color??'#111827'} onChange={e=>patchSettings(effect.id,{color:e.target.value})}/></label><label>Angle °<input type="number" value={effect.settings.angleDeg??19} onChange={e=>patchSettings(effect.id,{angleDeg:Number(e.target.value)||0})}/></label><label>Distance mm<input type="number" min="0" step=".05" value={effect.settings.distanceMm??.8} onChange={e=>patchSettings(effect.id,{distanceMm:Math.max(0,Number(e.target.value)||0)})}/></label><label>Size mm<input type="number" min=".02" step=".05" value={effect.settings.sizeMm??1.2} onChange={e=>patchSettings(effect.id,{sizeMm:Math.max(.02,Number(e.target.value)||.02)})}/></label></div><div className="card-property-grid"><label>Contour<select value={effect.settings.glossContour??'GAUSSIAN'} onChange={e=>patchSettings(effect.id,{glossContour:e.target.value as NonNullable<TextLayerEffect['settings']['glossContour']>})}><option value="LINEAR">Linear</option><option value="CONE">Cone</option><option value="CONE_INVERTED">Cone Inverted</option><option value="RING">Ring</option><option value="GAUSSIAN">Gaussian</option><option value="DOUBLE_RING">Double Ring</option><option value="ROUNDED_STEPS">Rounded Steps</option></select></label><label className="card-check-row"><input type="checkbox" checked={effect.settings.satinInvert===true} onChange={e=>patchSettings(effect.id,{satinInvert:e.target.checked})}/>Invert</label></div></>}
        {effect.type==='BEVEL_EMBOSS'&&<><div className="card-property-note"><strong>Bevel & Emboss</strong><span>Editable depth, surface technique and shared lighting controls.</span></div><div className="card-property-grid"><label>Style<select value={effect.settings.bevelStyle??'INNER_BEVEL'} onChange={e=>patchSettings(effect.id,{bevelStyle:e.target.value as NonNullable<TextLayerEffect['settings']['bevelStyle']>})}><option value="INNER_BEVEL">Inner Bevel</option><option value="OUTER_BEVEL">Outer Bevel</option><option value="EMBOSS">Emboss</option><option value="PILLOW_EMBOSS">Pillow Emboss</option><option value="STROKE_EMBOSS">Stroke Emboss</option></select></label><label>Technique<select value={effect.settings.bevelTechnique??'SMOOTH'} onChange={e=>patchSettings(effect.id,{bevelTechnique:e.target.value as NonNullable<TextLayerEffect['settings']['bevelTechnique']>})}><option value="SMOOTH">Smooth</option><option value="CHISEL_HARD">Chisel Hard</option><option value="CHISEL_SOFT">Chisel Soft</option></select></label><label>Depth %<input type="number" min="1" max="1000" value={effect.settings.depthPct??100} onChange={e=>patchSettings(effect.id,{depthPct:clamp(Number(e.target.value)||100,1,1000),depthMm:Math.max(.02,(effect.settings.sizeMm??.35)*clamp(Number(e.target.value)||100,1,1000)/100)})}/></label><label>Direction<select value={effect.settings.direction??'UP'} onChange={e=>patchSettings(effect.id,{direction:e.target.value as 'UP'|'DOWN'})}><option value="UP">Up</option><option value="DOWN">Down</option></select></label></div><div className="card-property-grid"><label>Size (mm)<input type="number" min=".02" step=".05" value={effect.settings.sizeMm??.35} onChange={e=>patchSettings(effect.id,{sizeMm:Math.max(.02,Number(e.target.value)||.02)})}/></label><label>Soften (mm)<input type="number" min="0" step=".05" value={effect.settings.softenMm??.1} onChange={e=>patchSettings(effect.id,{softenMm:Math.max(0,Number(e.target.value)||0)})}/></label><label>Gloss Contour<select value={effect.settings.glossContour??'LINEAR'} onChange={e=>patchSettings(effect.id,{glossContour:e.target.value as NonNullable<TextLayerEffect['settings']['glossContour']>})}><option value="LINEAR">Linear</option><option value="CONE">Cone</option><option value="CONE_INVERTED">Cone Inverted</option><option value="RING">Ring</option><option value="GAUSSIAN">Gaussian</option><option value="DOUBLE_RING">Double Ring</option><option value="ROUNDED_STEPS">Rounded Steps</option></select></label><label className="card-check-row"><input type="checkbox" checked={effect.settings.useGlobalLight!==false} onChange={e=>patchSettings(effect.id,{useGlobalLight:e.target.checked,angleDeg:e.target.checked?globalLight.angleDeg:(effect.settings.angleDeg??globalLight.angleDeg),altitudeDeg:e.target.checked?globalLight.altitudeDeg:(effect.settings.altitudeDeg??globalLight.altitudeDeg)})}/>Use Global Light</label></div><div className="card-property-grid"><label>Contour Strength %<input type="number" min="0" max="200" value={Math.round((effect.settings.contourStrength??1)*100)} onChange={e=>patchSettings(effect.id,{contourStrength:clamp(Number(e.target.value)||0,0,200)/100})}/></label><label>Texture<select value={effect.settings.texturePattern?.kind??'HATCH'} onChange={e=>patchSettings(effect.id,{texturePattern:{kind:e.target.value as "HATCH"|"DOT"|"CHECKER",foreground:"#ffffff",background:"#111827",scale:8,rotationDeg:45,opacity:1}})}><option value="HATCH">Hatch</option><option value="DOT">Dots</option><option value="CHECKER">Checker</option></select></label><label>Texture Depth %<input type="number" min="-100" max="100" value={Math.round((effect.settings.textureDepth??0)*100)} onChange={e=>patchSettings(effect.id,{textureDepth:clamp(Number(e.target.value)||0,-100,100)/100})}/></label><label className="card-check-row"><input type="checkbox" checked={effect.settings.textureInvert===true} onChange={e=>patchSettings(effect.id,{textureInvert:e.target.checked})}/>Invert Texture</label><label className="card-check-row"><input type="checkbox" checked={effect.settings.textureLinked!==false} onChange={e=>patchSettings(effect.id,{textureLinked:e.target.checked})}/>Link Texture</label></div><div className="card-property-grid"><label>Angle °<input type="number" min="-180" max="180" value={effect.settings.useGlobalLight!==false?globalLight.angleDeg:(effect.settings.angleDeg??120)} onChange={e=>{const angle=clamp(Number(e.target.value)||0,-180,180);if(effect.settings.useGlobalLight!==false)onGlobalLightChange(angle,globalLight.altitudeDeg);else patchSettings(effect.id,{angleDeg:angle})}}/></label><label>Altitude °<input type="number" min="0" max="90" value={effect.settings.useGlobalLight!==false?globalLight.altitudeDeg:(effect.settings.altitudeDeg??30)} onChange={e=>{const altitude=clamp(Number(e.target.value)||0,0,90);if(effect.settings.useGlobalLight!==false)onGlobalLightChange(globalLight.angleDeg,altitude);else patchSettings(effect.id,{altitudeDeg:altitude})}}/></label></div><div className="card-property-grid"><label>Highlight<input type="color" value={effect.settings.highlightColor??'#ffffff'} onChange={e=>patchSettings(effect.id,{highlightColor:e.target.value})}/></label><label>Highlight %<input type="number" min="0" max="100" value={Math.round((effect.settings.highlightOpacity??.75)*100)} onChange={e=>patchSettings(effect.id,{highlightOpacity:clamp(Number(e.target.value)||0,0,100)/100})}/></label><label>Highlight Blend<select value={effect.settings.highlightBlendMode??'SCREEN'} onChange={e=>patchSettings(effect.id,{highlightBlendMode:e.target.value as TextLayerEffect['blendMode']})}><option value="SCREEN">Screen</option><option value="NORMAL">Normal</option><option value="OVERLAY">Overlay</option><option value="SOFT_LIGHT">Soft Light</option></select></label></div><div className="card-property-grid"><label>Shadow<input type="color" value={effect.settings.shadowColor??'#111827'} onChange={e=>patchSettings(effect.id,{shadowColor:e.target.value})}/></label><label>Shadow %<input type="number" min="0" max="100" value={Math.round((effect.settings.shadowOpacity??.75)*100)} onChange={e=>patchSettings(effect.id,{shadowOpacity:clamp(Number(e.target.value)||0,0,100)/100})}/></label><label>Shadow Blend<select value={effect.settings.shadowBlendMode??'MULTIPLY'} onChange={e=>patchSettings(effect.id,{shadowBlendMode:e.target.value as TextLayerEffect['blendMode']})}><option value="MULTIPLY">Multiply</option><option value="NORMAL">Normal</option><option value="OVERLAY">Overlay</option><option value="SOFT_LIGHT">Soft Light</option></select></label></div></>}
...
function AdvancedTextProperties({element,update,artboard,template,mutate,availableFields,datasourceStatus}:{element:TextDesignElement;update:(f:(e:DesignElement)=>DesignElement)=>void;artboard:Artboard;template:DesignTemplate;mutate:(fn:(t:DesignTemplate)=>DesignTemplate,record?:boolean)=>void;availableFields:import('@document-tool/contracts').FieldDefinition[];datasourceStatus:string}){
  ...
  const metadataGlobalLight=((template.metadata??{}).textGlobalLight as {angleDeg?:number;altitudeDeg?:number}|undefined);
  const globalLight={angleDeg:metadataGlobalLight?.angleDeg??120,altitudeDeg:metadataGlobalLight?.altitudeDeg??30};
  const updateGlobalLight=(angleDeg:number,altitudeDeg:number)=>mutate(t=>({
    ...t,
    metadata:{...(t.metadata??{}),textGlobalLight:{angleDeg,altitudeDeg}},
    artboards:t.artboards.map(a=>({...a,elements:a.elements.map(el=>el.type==='TEXT'?{...el,style:{...el.style,layerEffects:(el.style.layerEffects??[]).map(effect=>effect.type==='BEVEL_EMBOSS'&&effect.settings.useGlobalLight!==false?{...effect,settings:{...effect.settings,angleDeg,altitudeDeg}}:effect)}}:el)}))
  }));
...
function SvgDielineImportPanel({template,artboard,selection,mutate,setSelection,setStatus}:{template:DesignTemplate;artboard:Artboard;selection:DesignSelectionState;mutate:(f:(t:DesignTemplate)=>DesignTemplate)=>void;setSelection:(value:DesignSelectionState)=>void;setStatus:(value:string)=>void}){
 ...
 const mapPanel=()=>{if(!selectedPaths.length){setStatus('Select the vector/region that bounds the panel first.');return;}mutate(t=>mapSelectionToPackagingPanel(t,artboard.id,selectedPaths.map(e=>e.id),role,{safeMarginMm:safe,bleedMm:bleed,flapKind}));setStatus(`${(role as string).replace(/_/g,' ')} panel mapped from selected geometry.`);};
...
const pageBorder=pageBorderSettings(artboard);
const patchPageBorder=(patch:Partial<PageBorderSettings>)=>mutate(t=>({...t,artboards:t.artboards.map(a=>a.id===artboard.id?{...a,metadata:{...(a.metadata??{}),pageBorder:{...pageBorderSettings(a),...patch}}}:a)}));
const resetPageBorder=()=>mutate(t=>({...t,artboards:t.artboards.map(a=>a.id===artboard.id?{...a,metadata:{...(a.metadata??{}),pageBorder:{...DEFAULT_PAGE_BORDER}}}:a)}));
return <div className="card-property-sections">
<Section sectionKey="GENERAL" title="General">
...
// @ts-ignore -- textShadowCss kept as fallback; superseded by textEffectShadowCss
...
function textLayerEffectsShadowCss(effects?:TextLayerEffect[]){const parts:string[]=[];for(const effect of effects??[]){if(!effect.enabled)continue;const opacity=effect.opacity??1,s=effect.settings;if(effect.type==='DROP_SHADOW')parts.push(`${s.offsetXmm??.5}mm ${s.offsetYmm??.5}mm ${Math.max(0,s.blurMm??.6)}mm ${colorWithOpacity(s.color??'#111827',opacity)}`);else if(effect.type==='OUTER_GLOW'){const c=colorWithOpacity(s.color??'#60a5fa',opacity),b=Math.max(0,s.blurMm??1.5);parts.push(`0 0 ${b}mm ${c}`);parts.push(`0 0 ${Math.max(.02,b*.45)}mm ${c}`)}else if(effect.type==='INNER_SHADOW'){const c=colorWithOpacity(s.color??'#111827',opacity),x=s.offsetXmm??.2,y=s.offsetYmm??.2,b=Math.max(0,s.blurMm??.4);parts.push(`${x}mm ${y}mm ${b}mm ${c}`);parts.push(`${-x*.4}mm ${-y*.4}mm ${Math.max(.02,b*.6)}mm ${c}`)}else if(effect.type==='INNER_GLOW'){const c=colorWithOpacity(s.color??'#ffffff',opacity),b=Math.max(0,s.blurMm??.8);parts.push(`0 0 ${Math.max(.02,b*.35)}mm ${c}`);parts.push(`0 0 ${Math.max(.02,b*.7)}mm ${colorWithOpacity(s.color??'#ffffff',opacity*.55)}`)}else if(effect.type==='SATIN'){const angle=(s.angleDeg??19)*Math.PI/180,d=Math.max(0,s.distanceMm??.8),size=Math.max(.02,s.sizeMm??1.2),inv=s.satinInvert?-1:1,c=colorWithOpacity(s.color??'#111827',opacity),x=Math.cos(angle)*d*inv,y=Math.sin(angle)*d*inv;parts.push(`${x}mm ${y}mm ${size}mm ${c}`);parts.push(`${-x}mm ${-y}mm ${Math.max(.02,size*.65)}mm ${colorWithOpacity(s.color??'#111827',opacity*.65)}`)}else if(effect.type==='BEVEL_EMBOSS'){const size=Math.max(.02,s.sizeMm??s.depthMm??.35),depth=Math.max(.01,size*Math.max(1,s.depthPct??100)/100),dir=s.direction==='DOWN'?-1:1,angle=(s.angleDeg??120)*Math.PI/180,alt=Math.max(0,Math.min(90,s.altitudeDeg??30))*Math.PI/180,styleFactor=s.bevelStyle==='OUTER_BEVEL'?1.2:s.bevelStyle==='EMBOSS'?1.35:s.bevelStyle==='PILLOW_EMBOSS'?0.85:s.bevelStyle==='STROKE_EMBOSS'?0.7:1,offset=Math.max(.02,depth*Math.max(.2,Math.cos(alt))*styleFactor),x=Math.cos(angle)*offset*dir,y=-Math.sin(angle)*offset*dir,tech=s.bevelTechnique??'SMOOTH',blur=tech==='CHISEL_HARD'?0:Math.max(.01,(s.softenMm??.1)*(tech==='CHISEL_SOFT'?1.6:1)),contour=s.glossContour??'LINEAR',contourBoost=(s.contourStrength??1)*(contour==='CONE'?1.15:contour==='CONE_INVERTED'?0.9:contour==='RING'?1.25:contour==='GAUSSIAN'?0.8:contour==='DOUBLE_RING'?1.35:contour==='ROUNDED_STEPS'?1.1:1),hi=opacity*(s.highlightOpacity??.75)*contourBoost,sh=opacity*(s.shadowOpacity??.75);parts.push(`${-x}mm ${-y}mm ${blur}mm ${colorWithOpacity(s.highlightColor??'#ffffff',Math.min(1,hi))}`);parts.push(`${x}mm ${y}mm ${blur}mm ${colorWithOpacity(s.shadowColor??'#111827',Math.min(1,sh))}`);const td=Math.abs(s.textureDepth??0);if(td>0){const sign=s.textureInvert?-1:1,tc=colorWithOpacity(sign>0?(s.highlightColor??'#ffffff'):(s.shadowColor??'#111827'),Math.min(.45,td*.4*opacity)),scale=(s.texturePattern?.scale??8)/100;for(const [tx,ty] of [[-.7,-.25],[.55,.2],[-.15,.65],[.35,-.6]])parts.push(`${tx*scale}mm ${ty*scale}mm 0 ${tc}`)}}}return parts.length?parts.join(', '):undefined}
