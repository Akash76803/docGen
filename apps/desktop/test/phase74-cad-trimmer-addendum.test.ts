...
describe('Phase 7.4 CAD Trimmer addendum',()=>{
 ...
 it('keeps Trimmer active after deletion and stages Escape before exit',()=>{expect(designer).not.toContain("deleteManualTrimRange();setInteractionMode('SELECT')");expect(designer).toContain('if (trimStartNodeId || trimEndNodeId || trimGeometry)');expect(designer).toContain('clearManualTrim();');expect(designer).toMatch(/else if\(interactionMode==='EDIT_PATH'\|\|interactionMode==='SCISSORS'\|\|interactionMode==='TRIMMER'\)\{endHistoryTransaction\(\);setInteractionMode\('SELECT'\)/);});
 ...
 it('provides compact CAD command states and closed-route side switching',()=>{expect(designer).toContain('TRIM — Select segment or first point');expect(designer).toContain('TRIM — Select second point');expect(designer).toContain('TRIM — Press Delete to remove selected segment');expect(designer).toContain('Switch Side');});
});
