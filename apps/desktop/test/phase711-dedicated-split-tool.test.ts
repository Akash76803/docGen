...
describe('Phase 7.11 dedicated Split tool wiring',()=>{
  ...
  it('keeps normal LINE and FLEXIBLE_LINE split behavior in place',()=>{
    ...
    expect(designer).toContain("if(interactionMode!=='FLEXIBLE_LINE')return extended");
  });
  ...
  it('relabels Trimmer only in the UI and clarifies destructive tool help',()=>{
    ...
    expect(contextToolbar).toContain('Subtract — remove the second/top selected path from the first/bottom selected path.');
